            ...
            <div class="col-12">
                <div class="card card-default">
                    {{-- <div class="card-header">
                        <h3 class="card-title">{{ __('รายการ') }}</h3>
                    </div> --}}
                    <div class="card-body">
        ...
        table = $dt.DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    targets : 8,
                    render: function ( data, type, full, meta ) {
                        return `<button type="button" class="btn btn-danger btn-xs btn-change-status" data-id="${full.id}" data-status="pending"><i class="fas fa-chevron-left"></i> ย้ายกลับไปตรวจสอบ</button>`;
                    }
                },
            ],
...
<script>
    reloadSummary = {
        init:function(){
            var url = "{!! route('widget.detail.ajax') !!}?status=confirm";
            $.ajax({
                url: url,
                method:'get',
                dataType:'json',
    ...
    changeStatus = {
        batch:function(){
            ...
            $.ajax({
                ...
                method:'post',
                dataType:'json',
                data: {_token:'{{ Session::token() }}', status: 'delivered', ids: ids, _method:'patch'},
                beforeSend: function() {
        ...
        little:function(id, status){
            var url = "{!! route('orders.details.status', ['detail_id' => ':id']) !!}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                method:'post',
                dataType:'json',
                data: {_token:'{{ Session::token() }}', status: status, id: id, _method:'patch'},
                beforeSend: function() {
                    $('body').append('<div class="preloader"><i class="fas fa-3x fa-sync-alt fa-spin"></i></div>');
                }
            })
...
<script>
    (function(window, document, $, undefined) {
        ...
        $(document).on('click', '.btn-change-status', function(e){
            ...
            Swal.fire({
                title: 'คุณแน่ใจใช่ไหม?',
                text: "ย้ายกลับไปตรวจสอบรายการอาหาร!",
                type: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
